...
func (s *SmartContract) SubmitOffer(ctx contractapi.TransactionContextInterface, userId string, offerId string, price float64, quantity int, isSeller bool, adminId string) (*Offer, error) {
	...
	if isSeller {
		...
		// 记录用户余额变动
		key,err = s.recordBalanceChange(ctx, userId, -depositAmount, user.Balance, fmt.Sprintf("报价:%s保证金", offerId), false, AdminName, false);
		if  err != nil {
		...
		// 记录管理员余额变动
		key,err = s.recordBalanceChange(ctx, adminId, depositAmount, admin.Balance, fmt.Sprintf("报价:%s的保证金", offerId), true, user.UserName, true);
		if  err != nil {
	...
	// 记录报价历史
	if err := s.recordOfferHistory(ctx, offer, "提交",user); err != nil {
...
func (s *SmartContract) updateOffer(ctx contractapi.TransactionContextInterface, sellerOffer, buyerOffer *Offer, contract *Contract, admin *PlatformAdmin) error {
	...
	if err := s.updateUser(ctx, sellerUser); err != nil {
		fmt.Println("更新卖方信息失败:", err)
		return err
	}
	if err := s.saveOffer(ctx, sellerOffer); err != nil {
		fmt.Println("存储卖方报价失败:", err)
		return err
	}
	...
	if err := s.updateUser(ctx, buyerUser); err != nil {
		fmt.Println("更新买方信息失败:", err)
		return err
	}
	if err := s.saveOffer(ctx, buyerOffer); err != nil {
		fmt.Println("存储买方报价失败:", err)
		return err
	}
...
func (s *SmartContract) updateUserOffer(ctx contractapi.TransactionContextInterface, user *User, offer *Offer, matchedQuantity int, admin *PlatformAdmin) {
	...
	if offer.Quantity == 0 {
		...
		if !offer.IsSeller {
			err:=s.refundBuyerDeposit(ctx, offer, admin, offer.Deposit,user)
			if err != nil {
				fmt.Println("退还保证金失败:", err)
				return
			}
		}
			
		err:=s.updateOfferHistory(ctx, offer, "完成",user)
		if err != nil {
			fmt.Println("更新报价历史失败:", err)
			return
		}
		...
		if !offer.IsSeller {
			...
			err:=s.refundBuyerDeposit(ctx, offer, admin, refundDeposit,user)
			if err != nil {
				fmt.Println("部分    退还保证金失败:", err)
				return
			}
		} 
		err:=s.updateOfferHistory(ctx, offer, "更新",user)
		if err != nil {
			fmt.Println("部分     更新报价历史失败:", err)
			return
		}
	}
}
...
func (s *SmartContract) updateOfferHistory(ctx contractapi.TransactionContextInterface, offer *Offer, action string,user *User) error {
	...
	fmt.Println("updateOfferHistory uniqueID:", uniqueID)
...
func (s *SmartContract) refundBuyerDeposit(ctx contractapi.TransactionContextInterface, offer *Offer, admin *PlatformAdmin, refundAmount float64,user *User) error {
	// user, err := s.getUser(ctx, offer.UserID)
	// if err != nil {
	// 	return err
	// }
	fmt.Println("into refundBuyerDeposit--------------refund amount:", refundAmount)
	...
	// 记录余额变动
	key,err := s.recordBalanceChange(ctx, user.UserID, refundAmount, user.Balance, fmt.Sprintf("退还保证金：%s", offer.OfferID), true, AdminName, false);
	if  err != nil {
		return err
	}
	user.BalanceHistory = append(user.BalanceHistory, key)
	...
	// 记录管理员余额变动
	key,err = s.recordBalanceChange(ctx, admin.AdminID, -refundAmount, admin.Balance, fmt.Sprintf("退还报价 %s 保证金", offer.OfferID), false, user.UserName, true); 
	if err != nil {
		return err
	}
...
func (s *SmartContract) SettleContract(ctx contractapi.TransactionContextInterface, adminId string) error {
	...
	for _, contractID := range admin.Contracts {
		...
		key,err := s.recordBalanceChange(ctx, buyerUser.UserID, -totalAmount, buyerUser.Balance, fmt.Sprintf("合同结算支出（合同ID：%s）", contract.ContractID), false, sellerUser.UserName, false); 
		if err != nil {
			return err
		}
		...
		key,err = s.recordBalanceChange(ctx, sellerUser.UserID, totalAmount - fee, sellerUser.Balance, fmt.Sprintf("合同结算收入（合同ID：%s，扣除手续费）", contract.ContractID), true, buyerUser.UserName, false);
		if  err != nil {
			return err
		}
		...
		// 平台收取手续费
		admin.Balance += fee
		key,err = s.recordBalanceChange(ctx, admin.AdminID, fee, admin.Balance, fmt.Sprintf("收取合同（ID：%s）结算手续费", contract.ContractID), true, sellerUser.UserName, true);
		if  err != nil {
			return err
		}
		admin.BalanceHistory = append(admin.BalanceHistory, key)
		...
		contract.SettledTime = getLocalTime()
...
func (s *SmartContract) ModifyOffer(ctx contractapi.TransactionContextInterface, userId string, offerId string, newPrice float64, newQuantity int, adminId string) error {
	...
	if !offer.IsSeller {
		...
		if newDeposit > oldDeposit {
			...
			// 记录保证金增加
			key,err := s.recordBalanceChange(ctx, user.UserID, -additionalDeposit, user.Balance, fmt.Sprintf("修改报价（ID:%s）追加保证金", offerId), false, AdminName, false); 
			if err != nil {
				return err
			}
			user.BalanceHistory = append(user.BalanceHistory, key)
			...
			// 记录管理员余额变化
			key,err = s.recordBalanceChange(ctx, admin.AdminID, additionalDeposit, admin.Balance, fmt.Sprintf("修改报价（ID:%s）追加保证金", offerId), true, user.UserName, true); 
			if err != nil {
				return err
			}
			admin.BalanceHistory = append(admin.BalanceHistory, key)
			...
			// 记录保证金退还
			key,err := s.recordBalanceChange(ctx, user.UserID, refundAmount, user.Balance, fmt.Sprintf("修改报价（ID:%s）退还保证金", offerId), true, AdminName, false);
			if  err != nil {
				return err
			}
			user.BalanceHistory = append(user.BalanceHistory, key)
			...
			// 记录管理员余额变化
			key,err = s.recordBalanceChange(ctx, admin.AdminID, -refundAmount, admin.Balance, fmt.Sprintf("修改报价（ID:%s）退还保证金", offerId), false, user.UserName, true);
			if  err != nil {
				return err
			}
			admin.BalanceHistory = append(admin.BalanceHistory, key)
...
func (s *SmartContract) CancelOffer(ctx contractapi.TransactionContextInterface, userId string, offerId string, adminId string) error {
	// 获取完整的Offer信息
	fmt.Println("into canceloffer")
	offer, err := s.getOffer(ctx, offerId)
	if err != nil {
		fmt.Println("获取报价信息失败:", err)
		return fmt.Errorf("获取报价信息失败: %w", err)
	}

	// 验证用户身份
	if offer.UserID != userId {
		fmt.Println("用户无权撤销报价:", offer.UserID, userId)
		return fmt.Errorf("用户 %s 无权撤销报价 %s", userId, offerId)
	}

	// 验证报价状态是否可撤销
	if offer.Status != OfferPending {
		fmt.Println("报价状态不允许撤销:", offer.Status)
	...
	if!offer.IsSeller {
		...
		if err := s.refundBuyerDeposit(ctx, offer, admin, offer.Deposit,user); err != nil {
			fmt.Println("退还保证金失败:", err)
			return err
		}
	}
	...
	// 创建报价撤销历史记录
	if err := s.recordOfferHistory(ctx, offer, "撤销",user); err != nil {
		fmt.Println("记录报价撤销历史失败:", err)
		return err
	}
	...
	// 更新账本中的Offer
	if err := s.saveOffer(ctx, offer); err != nil {
		fmt.Println("存储撤销报价信息失败:", err)
		return err
	}
	fmt.Println("撤销报价成功well done")
	return nil
}
...
func (s *SmartContract) recordBalanceChange(ctx contractapi.TransactionContextInterface, userId string, amount float64, rest float64, reason string, isIncome bool, userName string, isAdmin bool) (string, error ){
	// 生成唯一 ID，使用自增 ID
	...
	uniqueID := strconv.Itoa(id)
	fmt.Println("recordBalance uniqueID:", uniqueID)
	...
	fmt.Println("recordBalance存储账本成功")
	
	// // 根据用户类型更新相应的实体
	// if isAdmin {
	// 	// 管理员
	// 	user.BalanceHistory = append(user.BalanceHistory, balanceRecordKey)
	// 	if err := s.updateAdmin(ctx, user.(*PlatformAdmin)); err != nil {
	// 		fmt.Println("更新管理员信息失败:", err)
	...
	return balanceRecordKey,nil
}

// recordOfferHistory 记录报价历史
func (s *SmartContract) recordOfferHistory(ctx contractapi.TransactionContextInterface, offer *Offer, action string,user *User) error {
	// user, err := s.getUser(ctx, offer.UserID)
	// if err != nil {
	// 	return err
	// }
	...
	historyRecordKey, er := ctx.GetStub().CreateCompositeKey(OfferHistoryPrefix, []string{offer.OfferID, uniqueID})
	fmt.Println("recordOfferHistory key:", historyRecordKey)
	if er != nil {
		fmt.Println("Error creating composite key:", er)
		return er
	}
	// 序列化并存储历史记录
	historyRecordJSON, _ := json.Marshal(historyRecord)
